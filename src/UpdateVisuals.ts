...
export const visualUpdateBuildings = () => {
    ...
    if (G['buildingSubTab'] === 'coin') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            DOMCacheGetOrSet("buildtext" + (2 * i)).textContent = inMod("bbshards")?"BBSHARDS": "Coins/Sec: " + format((place.dividedBy(G['taxdivisor'])).times(40), 2) + " [" + format(percentage, 3) + "%]"
        }

        DOMCacheGetOrSet("buildtext11").textContent = inMod("bbshards")?"BBSHARDS": "Accelerators: " + format(player.acceleratorBought, 0, true) + " [+" + format(G['freeAccelerator'], 0, true) + "]"
        DOMCacheGetOrSet("buildtext12").textContent = inMod("bbshards")?"BBSHARDS": "Acceleration Power: " + ((G['acceleratorPower'] - 1) * (100)).toPrecision(4) + "% || Acceleration Multiplier: " + format(G['acceleratorEffect'], 2) + "x"
        DOMCacheGetOrSet("buildtext13").textContent = inMod("bbshards")?"BBSHARDS": "Multipliers: " + format(player.multiplierBought, 0, true) + " [+" + format(G['freeMultiplier'], 0, true) + "]"
        DOMCacheGetOrSet("buildtext14").textContent = inMod("bbshards")?"BBSHARDS": "Multiplier Power: " + G['multiplierPower'].toPrecision(4) + "x || Multiplier: " + format(G['multiplierEffect'], 2) + "x"
        DOMCacheGetOrSet("buildtext15").textContent = inMod("bbshards")?"BBSHARDS": "Accelerator Boost: " + format(player.acceleratorBoostBought, 0, true) + " [+" + format(G['freeAcceleratorBoost'], 0, false) + "]"
        DOMCacheGetOrSet("buildtext16").textContent = inMod("bbshards")?"BBSHARDS": "Reset Diamonds and Prestige Upgrades, but add " + (G['tuSevenMulti'] * (1 + player.researches[16] / 50) * (1 + CalcECC('transcend', player.challengecompletions[2]) / 100)).toPrecision(4) + "% Acceleration Power and 5 free Accelerators."
        DOMCacheGetOrSet("buyaccelerator").textContent = inMod("bbshards")?"BBSHARDS": "Cost: " + format(player.acceleratorCost) + " coins."
        DOMCacheGetOrSet("buymultiplier").textContent = inMod("bbshards")?"BBSHARDS": "Cost: " + format(player.multiplierCost) + " coins."
        DOMCacheGetOrSet("buyacceleratorboost").textContent = inMod("bbshards")?"BBSHARDS": "Cost: " + format(player.acceleratorBoostCost) + " Diamonds."

        // update the tax text
        let warning = "";
        if (player.reincarnationCount > 0.5 || inMod("supertax")) {
            warning = inMod("bbshards")?"BBSHARDS":`Your tax also caps your Coin gain at ${format(Decimal.pow(10, G['maxexponent'] - Decimal.log(G['taxdivisorcheck'], 10)))}/s.`
        }
        DOMCacheGetOrSet("taxinfo").textContent = inMod("bbshards")?"BBSHARDS":
    ...
    if (G['buildingSubTab'] === 'diamond') {
        ...
        if (player.resettoggle1 === 1 || player.resettoggle1 === 0) {
            ...
            DOMCacheGetOrSet("autoprestige").textContent = inMod("bbshards")?"BBSHARDS": "Prestige when your Diamonds can increase by a factor " + format(Decimal.pow(10, player.prestigeamount)) + " [Toggle number above]. Current Multiplier: " + format(p) + "."
        }
        if (player.resettoggle1 === 2) {
            DOMCacheGetOrSet("autoprestige").textContent = inMod("bbshards")?"BBSHARDS": "Prestige when the autotimer is at least " + (player.prestigeamount) + " real-life seconds. [Toggle number above]. Current timer: " + format(G['autoResetTimers'].prestige, 1) + "s."
        }
        // update the crystal tax text
        DOMCacheGetOrSet("crystaltaxinfo").textContent = inMod("bbshards")?"BBSHARDS":
    ...
    if (G['buildingSubTab'] === 'mythos') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            DOMCacheGetOrSet("transcendtext" + (2 * i - 1)).textContent = inMod("bbshards")?"BBSHARDS": names[i] + ": " + format(player[`${ith}OwnedMythos` as const], 0, true) + " [+" + format(player[`${ith}GeneratedMythos` as const], 2) + "]"
            DOMCacheGetOrSet("transcendtext" + (2 * i)).textContent = inMod("bbshards")?"BBSHARDS": perSecNames[i] + ": " + format((place).times(40), 2)
            DOMCacheGetOrSet("buymythos" + i).textContent = inMod("bbshards")?"BBSHARDS": "Cost: " + format(player[`${ith}CostMythos` as const], 2) + " Mythos"
        }

        if (player.resettoggle2 === 1 || player.resettoggle2 === 0) {
            DOMCacheGetOrSet("autotranscend").textContent = inMod("bbshards")?"BBSHARDS": "Prestige when your Mythos can increase by a factor " + format(Decimal.pow(10, player.transcendamount)) + " [Toggle number above]. Current Multiplier: " + format(Decimal.pow(10, Decimal.log(G['transcendPointGain'].add(1), 10) - Decimal.log(player.transcendPoints.add(1), 10)), 2) + "."
        }
        if (player.resettoggle2 === 2) {
            DOMCacheGetOrSet("autotranscend").textContent = inMod("bbshards")?"BBSHARDS": "Transcend when the autotimer is at least " + (player.transcendamount) + " real-life seconds. [Toggle number above]. Current timer: " + format(G['autoResetTimers'].transcension, 1) + "s."
        }

        DOMCacheGetOrSet("mythostaxinfo").textContent = inMod("bbshards")?"BBSHARDS":
    ...
    if (G['buildingSubTab'] === 'particle') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            DOMCacheGetOrSet(`reincarnationtext${i}`).textContent = inMod("bbshards")?"BBSHARDS":
                `${names[i-1]}: ${format(player[`${ith}OwnedParticles` as const], 0, true)} [+${format(player[`${ith}GeneratedParticles` as const], 2)}]`;
            DOMCacheGetOrSet(`reincarnationtext${i+5}`).textContent = inMod("bbshards")?"BBSHARDS":
                `${perSecNames[i-1]}: ${format((place).times(40), 2)}`;
            DOMCacheGetOrSet(`buyparticles${i}`).textContent = inMod("bbshards")?"BBSHARDS":
                `Cost: ${format(player[`${ith}CostParticles` as const], 2)} Particles`;
        }

        DOMCacheGetOrSet("reincarnationshardinfo").textContent = inMod("bbshards")?"BBSHARDS": "You have " + format(player.reincarnationShards, 2) + " Atoms, providing " + G['buildingPower'].toPrecision(4) + " Building Power. Multiplier to Coin Production: " + format(G['reincarnationMultiplier'])
        DOMCacheGetOrSet("reincarnationCrystalInfo").textContent = inMod("bbshards")?"BBSHARDS": "Thanks to Research 2x14, you also multiply Crystal production by " + format(Decimal.pow(G['reincarnationMultiplier'], 1 / 50), 3, false)
        DOMCacheGetOrSet("reincarnationMythosInfo").textContent = inMod("bbshards")?"BBSHARDS": "Thanks to Research 2x15, you also multiply Mythos Shard production by " + format(Decimal.pow(G['reincarnationMultiplier'], 1 / 250), 3, false)

        if (player.resettoggle3 === 1 || player.resettoggle3 === 0) {
            DOMCacheGetOrSet("autoreincarnate").textContent = inMod("bbshards")?"BBSHARDS": "Reincarnate when your Particles can increase by a factor " + format(Decimal.pow(10, player.reincarnationamount)) + " [Toggle number above]. Current Multiplier: " + format(Decimal.pow(10, Decimal.log(G['reincarnationPointGain'].add(1), 10) - Decimal.log(player.reincarnationPoints.add(1), 10)), 2) + "."
        }
        if (player.resettoggle3 === 2) {
            DOMCacheGetOrSet("autoreincarnate").textContent = inMod("bbshards")?"BBSHARDS": "Reincarnate when the autotimer is at least " + (player.reincarnationamount) + " real-life seconds. [Toggle number above]. Current timer: " + format(G['autoResetTimers'].reincarnation, 1) + "s."
        }

        DOMCacheGetOrSet("particletaxinfo").textContent = inMod("bbshards")?"BBSHARDS":
    ...
    if (G['buildingSubTab'] === 'tesseract') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            DOMCacheGetOrSet("ascendText" + i).textContent = inMod("bbshards")?"BBSHARDS": names[i] + ": " + format(player[ascendBuildingI]['owned'], 0, true) + " [+" + format(player[ascendBuildingI]['generated'], 2) + "]"
            DOMCacheGetOrSet("ascendText" + (5 + i)).textContent = inMod("bbshards")?"BBSHARDS":
                perSecNames[i] + ": " + format(((G['ascendBuildingProduction'] as { [key: string]: Decimal })[G['ordinals'][i - 1]]), 2)
            DOMCacheGetOrSet("buyTesseracts" + i).textContent = inMod("bbshards")?"BBSHARDS": "Cost: " + format(player[ascendBuildingI]['cost'], 0) + " Tesseracts"
        }

        DOMCacheGetOrSet("tesseractInfo").textContent = inMod("bbshards")?"BBSHARDS": "You have " + format(player.wowTesseracts) + " Wow! Tesseracts. Gain more by beating Challenge 10 on each Ascension."
        DOMCacheGetOrSet("ascendShardInfo").textContent = inMod("bbshards")?"BBSHARDS": "You have a mathematical constant of " + format(player.ascendShards, 2) + ". Taxes are divided by " + format(Math.pow(Decimal.log(player.ascendShards.add(1), 10) + 1, 1 + .2 / 60 * player.challengecompletions[10] * player.upgrades[125] + 0.1 * player.platonicUpgrades[5] + 0.2 * player.platonicUpgrades[10] + (G['platonicBonusMultiplier'][5] - 1)), 4, true) + "."
        DOMCacheGetOrSet("autotessbuyeramount").textContent = inMod("bbshards")?"BBSHARDS": "Auto buyer will save at least " + format(player.tesseractAutoBuyerAmount) + " tesseracts. [Enter number above]."
        DOMCacheGetOrSet("tessertaxinfo").textContent = inMod("bbshards")?"BBSHARDS":
...
export const visualUpdateRunes = () => {
    ...
    if (G['runescreen'] === 'runes') { //Placeholder and place work similarly to buildings, except for the specific Talismans.
        ...
        DOMCacheGetOrSet("offeringCount").textContent = inMod("bbshards")?"BBSHARDS": "You have " + format(player.runeshards, 0, true) + " Offerings."
        ...
        for (let i = 1; i <= 7; i++) { //First one updates level, second one updates TNL, third updates orange bonus levels
            ...
            DOMCacheGetOrSet('rune' + i + 'exp').textContent = inMod("bbshards")?"BBSHARDS": (runeLevel < maxLevel ? "+1 in " + format(calculateRuneExpToLevel(i - 1) - player.runeexp[i - 1], 2) + " EXP" : "Max level!")
            if (i <= 5) DOMCacheGetOrSet('bonusrune' + i).textContent = inMod("bbshards")?"BBSHARDS":" [Bonus: " + format(7 * player.constantUpgrades[7] + Math.min(1e7, player.antUpgrades[8]! + G['bonusant9']) + place) + "]"
            else DOMCacheGetOrSet('bonusrune' + i).textContent = inMod("bbshards")?"BBSHARDS":"[Bonus: Nope!]"
            DOMCacheGetOrSet('rune' + i + 'level').childNodes[0].textContent = inMod("bbshards")?"BBSHARDS":"Level: " + format(player.runelevels[i - 1]) + "/" + format(calculateMaxRunes(i))
            DOMCacheGetOrSet('rune' + i + 'exp').textContent = inMod("bbshards")?"BBSHARDS":"+1 in " + format(calculateRuneExpToLevel(i - 1) - player.runeexp[i - 1], 2) + " EXP"
            if (i <= 5) DOMCacheGetOrSet('bonusrune' + i).textContent = inMod("bbshards")?"BBSHARDS":" [Bonus: " + format(7 * player.constantUpgrades[7] + Math.min(1e7, player.antUpgrades[8]! + G['bonusant9']) + place) + "]"
            else DOMCacheGetOrSet('bonusrune' + i).textContent = inMod("bbshards")?"BBSHARDS":"[Bonus: Nope!]"
    ...
    if (G['runescreen'] === 'blessings') {
        ...
        let t = 0;
        for (let i = 1; i <= 5; i++) {
            DOMCacheGetOrSet(`runeBlessingLevel${i}Value`).textContent = inMod("bbshards")?"BBSHARDS": format(player.runeBlessingLevels[i], 0, true)
            DOMCacheGetOrSet(`runeBlessingPower${i}Value1`).textContent = inMod("bbshards")?"BBSHARDS": format(G['runeBlessings'][i])
            ...
            DOMCacheGetOrSet(`runeBlessingPurchaseAmount${i}`).textContent = inMod("bbshards")?"BBSHARDS": format(Math.max(1, levelsPurchasable))
            DOMCacheGetOrSet(`runeBlessingPurchaseCost${i}`).textContent = inMod("bbshards")?"BBSHARDS": format(Math.max(G['blessingBaseCost'] * (1 + player.runeBlessingLevels[i]), calculateSummationLinear(player.runeBlessingLevels[i], G['blessingBaseCost'], player.runeshards, player.runeBlessingBuyAmount)[1]))
            if (i === 5) {
                t = 1
            }
            DOMCacheGetOrSet(`runeBlessingPower${i}Value2`).textContent = inMod("bbshards")?"BBSHARDS": format(1 - t + blessingMultiplierArray[i] * G['effectiveRuneBlessingPower'][i], 4, true)
        }
    }
    ...
    if (G['runescreen'] === 'spirits') {
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            DOMCacheGetOrSet(`runeSpiritPurchaseAmount${i}`).textContent = inMod("bbshards")?"BBSHARDS": format(Math.max(1, levelsPurchasable))
            DOMCacheGetOrSet(`runeSpiritPurchaseCost${i}`).textContent = inMod("bbshards")?"BBSHARDS": format(Math.max(G['spiritBaseCost'] * (1 + player.runeSpiritLevels[i]), calculateSummationLinear(player.runeSpiritLevels[i], G['spiritBaseCost'], player.runeshards, player.runeSpiritBuyAmount)[1]))
            DOMCacheGetOrSet(`runeSpiritPower${i}Value2`).textContent = inMod("bbshards")?"BBSHARDS": format(1 - subtract[i] + spiritMultiplierArray[i] * G['effectiveRuneSpiritPower'][i], 4, true)
        }
    }
}
...
export const visualUpdateCorruptions = () => {
    ...
    DOMCacheGetOrSet("corruptionBankValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[0]);
    DOMCacheGetOrSet("corruptionScoreValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[1], 0, true);
    DOMCacheGetOrSet("corruptionMultiplierValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[2], 1, true);
    DOMCacheGetOrSet("corruptionBonusMultiplierValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[9], 2, true);
    DOMCacheGetOrSet("corruptionTotalScore").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[3], 0, true);
    DOMCacheGetOrSet("corruptionCubesValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[4], 0, true);
    DOMCacheGetOrSet("corruptionTesseractsValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[5]);
    DOMCacheGetOrSet("corruptionHypercubesValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[6]);
    DOMCacheGetOrSet("corruptionPlatonicCubesValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[7]);
    DOMCacheGetOrSet("corruptionHepteractsValue").textContent = inMod("bbshards")?"BBSHARDS":format(metaData[8]);
    DOMCacheGetOrSet("corruptionAntExponentValue").textContent = inMod("bbshards")?"BBSHARDS":format((1 - 0.9 / 90 * sumContents(player.usedCorruptions)) * G['extinctionMultiplier'][player.usedCorruptions[7]], 3);
    DOMCacheGetOrSet("corruptionSpiritBonusValue").textContent = inMod("bbshards")?"BBSHARDS":format(calculateCorruptionPoints()/400,2,true);
    DOMCacheGetOrSet("corruptionAscensionCount").style.display = ascCount > 1 ? 'block' : 'none';

    if (ascCount > 1) {
        DOMCacheGetOrSet("corruptionAscensionCountValue").textContent = inMod("bbshards")?"BBSHARDS": format(calcAscensionCount());
    }
}
...
export const visualUpdateSettings = () => {
    ...
    DOMCacheGetOrSet("transcensionCountStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.transcendCount, 0, true)
    DOMCacheGetOrSet("reincarnationCountStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.reincarnationCount, 0, true)
    DOMCacheGetOrSet("fastestPrestigeStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(1000 * player.fastestprestige) + "ms"
    DOMCacheGetOrSet("fastestTranscensionStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(1000 * player.fastesttranscend) + "ms"
    DOMCacheGetOrSet("fastestReincarnationStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(1000 * player.fastestreincarnate) + "ms"
    DOMCacheGetOrSet("mostOfferingStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.maxofferings)
    DOMCacheGetOrSet("mostObtainiumStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.maxobtainium)
    DOMCacheGetOrSet("mostObtainiumPerSecondStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.maxobtainiumpersecond, 2, true)
    DOMCacheGetOrSet("runeSumStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(G['runeSum'])
    DOMCacheGetOrSet("obtainiumPerSecondStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.obtainiumpersecond, 2, true)
    DOMCacheGetOrSet("ascensionCountStatistic").childNodes[1].textContent = inMod("bbshards")?"BBSHARDS": format(player.ascensionCount, 0, true)

    DOMCacheGetOrSet("saveString").textContent = inMod("bbshards")?"BBSHARDS":
    ...
    DOMCacheGetOrSet("quarktimerdisplay").textContent = inMod("bbshards")?"BBSHARDS": format((3600 / (quarkData.perHour) - (player.quarkstimer % (3600.00001 / (quarkData.perHour)))), 2) + "s until +" + format(patreonLOL, 2, true) + " export Quark"
        ...
        if (shopItem.type === shopUpgradeTypes.UPGRADE) {
            ...
            if (shopItem.maxLevel === 1) {
                player.shopUpgrades[key] === shopItem.maxLevel ?
                    DOMCacheGetOrSet(`${key}Level`).textContent = inMod("bbshards")?"BBSHARDS": "Bought!":
                    DOMCacheGetOrSet(`${key}Level`).textContent = inMod("bbshards")?"BBSHARDS": "Not Bought!"
            }
