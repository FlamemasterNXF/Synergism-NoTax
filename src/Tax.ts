...
export const calculatetax = () => {
    ...
    compareC = Decimal.pow(G['maxexponent'], 2).div(inMod("supertax")?1:550);
    ...
    if (!inMod("notax")){
        ...
        if (inMod("supertax")) {
            ...
            compareB.div(Object.keys(player.upgrades).filter(x => player.upgrades[Number(x)]==1 && ((Number(x)>=3&&Number(x)<=4)||(Number(x)>=23&&Number(x)<=24))).length*1.25+1)
            compareC.div(Object.keys(player.upgrades).filter(x => player.upgrades[Number(x)]==1 && ((Number(x)>=3&&Number(x)<=4)||(Number(x)>=23&&Number(x)<=24))).length*1.25+1)
            if (exponent>=200) exponent = Math.pow(exponent, 2)/200;
        }

        G['taxdivisor'] = Decimal.pow(base, (compareB).times(exponent)).times(inMod("supertax")?(G["produceTotal"].plus(1).log10()+1):1)
        G['taxdivisorcheck'] = Decimal.pow(base, (compareC).times(exponent)).times(inMod("supertax")?(G["produceTotal"].plus(1).log10()+1):1)
    }
    else{
